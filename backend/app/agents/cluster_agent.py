...
def generate_clusters(db: Session, project_id: int = 1) -> list[InsightCluster]:
    items = db.query(FeedbackItem).filter(FeedbackItem.project_id == project_id).all()
    ...
    for module, rows in groups.items():
        ...
        existing = db.query(InsightCluster).filter_by(project_id=project_id, product_module=module).first()
        cluster = existing or InsightCluster(project_id=project_id, product_module=module, cluster_name=f"{module}体验痛点")
