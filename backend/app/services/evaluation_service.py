...
def overview(db: Session, project_id: int = 1):
    total = db.query(AgentRun).filter_by(project_id=project_id).count()
    success = db.query(AgentRun).filter_by(project_id=project_id, status="success").count()
    steps = db.query(AgentStep).count()
    return {
...
def retrieval_metrics(db: Session, project_id: int = 1):
    rows = db.query(RetrievalLog).all()
    total = len(rows)
    opps = db.query(Opportunity).filter_by(project_id=project_id).all()
...
def compression_metrics(db: Session):
    rows = db.query(CompressionLog).all()
    total = len(rows)
    by_type = {k: v for k, v in db.query(CompressionLog.compression_type, func.count()).group_by(CompressionLog.compression_type).all()}
    return {
...
def quality_metrics(db: Session, project_id: int = 1):
    ...
    required = ["## 1. 背景与问题", "## 2. 目标用户", "## 3. 用户故事", "## 4. 需求范围", "## 5. 功能流程", "## 6. 验收标准", "## 7. 埋点指标", "## 8. 风险点", "## 9. 后续迭代建议"]
