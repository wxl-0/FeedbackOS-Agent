...
def list_conversations(db: Session, project_id: int = 1):
    ...
    return [serialize_conversation(row, db.query(ConversationMessage).filter_by(conversation_id=row.id).count()) for row in rows]
...
def serialize_conversation(row: Conversation, message_count: int = 0):
    return {
        "id": row.id,
