...
@router.post("")
async def upload(conversation_id: str, file: UploadFile = File(...), db: Session = Depends(get_db)):
    item = await save_upload(db, file, conversation_id=conversation_id)
    return serialize_upload(item)


@router.get("/files")
def files(conversation_id: str | None = None, db: Session = Depends(get_db)):
    q = db.query(UploadedFile)
    if conversation_id:
        q = q.filter(UploadedFile.conversation_id == conversation_id)
    return [serialize_upload(f) for f in q.order_by(UploadedFile.id.desc()).all()]
